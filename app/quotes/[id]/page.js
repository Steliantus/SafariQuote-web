  ...
  const [{ data: quote }, { data: travelers }, profile] = await Promise.all([
        ...
        supabase.auth.getUser().then(async ({ data }) => {
                ...
                if (tErr) {
                  ...
                  return { tenantId: p.tenant_id, sto_discount_pct: tFallback?.sto_discount_pct, is_master_rate_source: null, companyName: tFallback?.company_name || "" };
                }
                return { tenantId: p.tenant_id, sto_discount_pct: t?.sto_discount_pct, is_master_rate_source: t?.is_master_rate_source ?? null, companyName: t?.company_name || "" };
        }),
      ]);
